...
class ClusterOperations:
    ...
    async def resolve_resource(self, group, version, kind, name, config, operations, delete=False, force=False):
...
class ResourceController:
    def __init__(self, config_ops: ClusterOperations, group, version, kind, name, validate=False):
    ...
    async def acquire_object(self):
        initial_ops = await self.config_ops.lock_resource(self.group, self.version, self.kind, name=self.name)
    ...
    async def next_operation_batch(self):
        ...
        if force:
            print('Warning: release config lock because not all operations were processed')
    ...
    async def get_current_config(self):
        if self.have_current_config == False:
            self.current_config = await self.config_ops.read_resource(self.group, self.version, self.kind, self.locked_config_name)
